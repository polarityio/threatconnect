...
polarity.export = PolarityComponent.extend({
  ...
  actions: {
    ...
    refreshIntegrations(issueIndex) {
      if (issueIndex !== undefined) {
        this.setIssueState(issueIndex, 'spinRefresh', true);
      }

      this.send('setIntegrationSelection', issueIndex);
    ...
    toggleAllIntegrations(issueIndex) {
      let integrations = this.get(`pagedPagingData.${issueIndex}.__state.integrations`);
      const hasUnselected = integrations.some((int) => !int.selected);

      integrations.forEach((integration, index) => {
        this.set(`pagedPagingData.${issueIndex}.__state.integrations.${index}.selected`, hasUnselected);
      });
